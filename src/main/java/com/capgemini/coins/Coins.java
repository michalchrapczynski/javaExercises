...
public class Coins {
	public static int solution(List<Integer> coins) {
		...
		for (int i = 1; i < coins.size() - 1 && changeValue; i++) {
			if (coins.get(i) != coins.get(i - 1) || coins.get(i) != coins.get(i + 1) && changeValue) {
				coins.set(i, coins.get(i - 1));
				changeValue = false;
				}

			if (coins.get(coins.size() - 2) != coins.get(coins.size() - 3)
					|| coins.get(coins.size() - 2) != coins.get(coins.size() - 1) && changeValue) {
				coins.set(coins.size() - 2, coins.get(coins.size() - 3));
				changeValue = false;
			}

				
			if (coins.get(0) != (coins.get(i + 1)) && changeValue) {
				coins.set(0, coins.get(i + 1));
					changeValue = false;
				}
				
			if (coins.get(coins.size() - 1) != coins.get(coins.size() - 2) && changeValue) {
				coins.set(coins.size() - 1, coins.get(coins.size() - 2));
					changeValue = false;
				}
